...
export const SupportedEVMNetworks = x402SupportedNetworks.filter(network => { // TODO_CHANGE: remove all
    const config = getNetworkConfig(network);
    return config?.architecture === 'evm';
});

export const SupportedSolanaNetworks = x402SupportedNetworks.filter(network => {
    const config = getNetworkConfig(network)
...
export const PaymentPayloadSchema = z.object({
    ...
    payload: z.union([ExactEvmPayloadSchema]),
});
...
export function encodePayment(payment: PaymentPayload): string {
    ...
    // svm
    //if (SupportedSolanaNetworks.includes(payment.network)) {
...
export async function createPaymentHeader(
    ...
    if (
        ...
        SupportedEVMNetworks.includes(paymentRequirements.network) // TODO_CHANGE: change to solana
    ) {
    ...
    if (
        paymentRequirements.scheme === "exact" &&
        SupportedSolanaNetworks.includes(paymentRequirements.network) // TODO_CHANGE: change to solana
    ) {
