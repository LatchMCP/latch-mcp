...
import {
  ...
  ToolInputSchema,
  type ToolFromMcpServerWithStats,
} from "@/types/mcp";
...
export function ToolExecutionModal({
  ...
  const needsBrowserConnection =
    requiresBrowserConnection &&
    (!isBrowserWalletConnected ||
      connectedWalletAddress?.toLowerCase() !== walletAddress?.toLowerCase());
  const needsWalletClient = requiresBrowserConnection; // Only external wallets need wagmi wallet client

  // Overall connection status
  const isConnected =
    hasAccountWallets &&
    (!needsWalletClient || hasWalletClient) &&
    !needsBrowserConnection;

  // Create stable tool reference to avoid infinite loops
  const toolInputSchemaString = useMemo(
  ...
  const [mcpToolsCollection, setMcpToolsCollection] =
    useState<MCPToolsCollection>({});
  const [isInitialized, setIsInitialized] = useState(false);
  const [isSwitchingNetwork, setIsSwitchingNetwork] = useState(false);
  const [showPrettyJson, setShowPrettyJson] = useState(true);
  const [jsonEditorStates, setJsonEditorStates] = useState<
  ...
  const getActivePricing = useCallback((): PricingEntry[] => {
    ...
    return (stableTool.pricing as PricingEntry[]).filter(
      (p) => p.active === true
    );
  }, [stableTool]);
  ...
  const getRequiredNetwork = useCallback((): string | null => {
    ...
    const selectedPricing =
      activePricing[selectedPricingTier] || activePricing[0];
  ...
  const handleNetworkSwitch = async () => {
    ...
    try {
      ...
      console.log(
        `[Network Switch] Successfully switched to ${requiredNetwork}`
      );
      ...
      setExecution({
        status: "error",
        error: `Failed to switch to ${requiredNetwork}: ${
  ...
  const formatCurrency = useCallback(
    (amount: string | number, currency: string, network?: string) => {
      ...
      if (currency.startsWith("0x") && currency.length === 42) {
        return `${num.toFixed(6)} ${currency.slice(0, 6)}...${currency.slice(
          -4
        )}`;
      }
  ...
  const getToolProperties = useCallback(
    (toolArg: ToolFromMcpServerWithStats): Record<string, InputProperty> => {
  ...
  const getRequiredFields = useCallback(
    (toolArg: ToolFromMcpServerWithStats): string[] => {
      // First try to get from MCP client tools (if available and initialized)
      if (isInitialized && mcpToolsCollection[toolArg.name]) {
        const mcpTool = mcpToolsCollection[toolArg.name] as MCPToolFromClient;
  ...
  const hasToolInputs = useCallback(
    (toolArg: ToolFromMcpServerWithStats): boolean => {
  ...
  useEffect(() => {
    if (!stableTool || !isInitialized || !mcpToolsCollection[stableTool.name])
      return;
    ...
    const initializeMcpClient = async () => {
      try {
        ...
        if (activeWallet?.walletType === "external") {
          ...
          console.log(
            "Using managed wallet with dummy account (server-side payment):",
            {
        ...
        if (tools[stableTool.name]) {
          ...
          const mcpTool = tools[
            stableTool.name
          ] as unknown as MCPToolFromClient;
          ...
          console.warn(
            `Warning: Tool "${stableTool.name}" not found in MCP server tools`
          );
        }
      } catch (error) {
  ...
  const executeTool = async () => {
    ...
    try {
      ...
      console.log("ThisIsDebug", mcpTool);
      ...
      console.log(
        `[Tool Execution] Starting execution for: ${stableTool.name}`
      );
      ...
      console.log(
        `[Tool Execution] MCP tool schema:`,
        mcpTool.parameters?.jsonSchema || mcpTool.inputSchema?.jsonSchema
      );
      ...
      if (typeof result === "object" && result !== null) {
        ...
        transactionId =
          (resultObj.transactionId as string) ||
      ...
      return (
        <div key={fullPath} className="space-y-3">
          <div className="space-y-2">
            <label
              className={`text-sm font-medium flex items-center gap-1 ${themeClasses.text.primary}`}
            >
          ...
          <div className="ml-4 pl-4 border-l-2 border-gray-200 dark:border-gray-700 space-y-3">
            {Object.entries(inputProp.properties).map(
              ([propertyName, propertySchema]) => {
                const propertyValue = objectValue[propertyName];
                const isPropertyRequired =
                  objectRequiredFields.includes(propertyName);
  ...
  const renderPrimitiveField = (
      ...
      return (
        <div key={propertyName} className="space-y-2">
          ...
          <Input
            ...
            value={String(fieldValue)}
            onChange={(e) =>
              updateProperty(propertyName, parseFloat(e.target.value) || "")
            }
    ...
    const isLongText =
      propertySchema.description && propertySchema.description.length > 100;
  ...
  const renderInputField = (inputName: string, inputProp: InputProperty) => {
      ...
      return (
        <div key={inputName} className="space-y-2">
          ...
          <Input
            ...
            value={String(currentValue)}
            onChange={(e) =>
              updateToolInput(inputName, parseFloat(e.target.value) || "")
            }
    ...
    // Default to text input
    const isLongText =
      inputProp.description && inputProp.description.length > 100;
  ...
  const renderPricingSection = () => {
    ...
    const selectedPricing =
      activePricing[selectedPricingTier] || activePricing[0];
    const amount =
      parseFloat(selectedPricing?.maxAmountRequiredRaw || "0") /
      Math.pow(10, selectedPricing?.tokenDecimals || 0);
    ...
    return (
      <div className="space-y-2">
        ...
        <div>
          <div className="flex items-center justify-between p-3  bg-[rgba(245,245,107,0.10)] rounded-lg">
            <div className="flex items-center gap-2">
              <span className="font-medium text-sm">
                {formatCurrency(
                  amount,
                  selectedPricing?.assetAddress || "",
                  selectedPricing?.network || ""
                )}
              </span>
              <span className="text-[#FAFABB] dark:text-gray-400 text-sm">
                on {selectedPricing?.network}
              </span>
            </div>
            ...
            {hasMultipleTiers && (
              <Popover
                ...
                onOpenChange={setPricingPopoverOpen}
              >
                  ...
                  <div className="p-4">
                    ...
                    <div className="space-y-2">
                      {activePricing.map((pricing, index) => {
                        const tierAmount =
                          parseFloat(pricing.maxAmountRequiredRaw) /
                          Math.pow(10, pricing.tokenDecimals);
                        return (
                          <div
                            key={pricing.id}
                            onClick={() => {
    ...
    if (!hasAccountWallets) {
      return (
        <div className="space-y-2">
          ...
          <div
            className={`ml-6 p-3 rounded-lg border ${themeClasses.background.warning}`}
          >
            <div className="flex items-center gap-2">
      ...
      <div className="space-y-2">
        ...
        <div>
          <div className="flex items-center justify-between p-3 bg-[rgba(245,245,107,0.10)] rounded-lg">
            ...
            <div className="flex items-center gap-2">
              {needsBrowserConnection && <ConnectButton />}
  ...
  const renderPaymentPreview = () => {
    const activePricing = getActivePricing();
    if (!stableTool?.isMonetized || !activePricing.length || !isConnected)
      return null;

    const selectedPricing =
      activePricing[selectedPricingTier] || activePricing[0];
    const amount =
      parseFloat(selectedPricing.maxAmountRequiredRaw) /
      Math.pow(10, selectedPricing.tokenDecimals);
    ...
    return (
      <div className="text-center text-sm text-gray-600 dark:text-gray-400 py-2">
        ...
        via{" "}
        <span className="font-medium">
          {activeWallet?.provider || "your wallet"}
        </span>
      </div>
    );
  };
  ...
  const renderNetworkSwitchPrompt = () => {
    ...
    return (
      <div
        className={`p-3 rounded-lg border ${themeClasses.background.warning}`}
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs">
                Switch to {getRequiredNetwork()} to execute this tool
              </p>
            </div>
          </div>
          <Button
    ...
    if (execution.status === "success" && execution.result) {
      ...
      return (
        <div className="space-y-3">
          ...
          <div
            className={`rounded-md border max-h-60 overflow-hidden ${themeClasses.background.code}`}
          >
            <pre
              className={`text-xs p-3 max-h-60 overflow-auto whitespace-pre-wrap ${themeClasses.text.primary}`}
            >
              {formatResult()}
            </pre>
          </div>
          ...
          <div className="space-y-3 pt-2">
            {execution.transactionId && (
              <div className="text-center text-xs text-gray-500 dark:text-gray-400">
                Transaction ID:{" "}
                <span className="font-mono">{execution.transactionId}</span>
              </div>
            )}
        ...
        <DrawerContent className={`max-h-[85vh] ${themeClasses.modal}`}>
          <DrawerHeader>
            ...
            <DrawerDescription>
              {stableTool
                ? (isInitialized &&
                    (mcpToolsCollection[stableTool.name] as MCPToolFromClient)
                      ?.description) ||
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        className={`max-w-2xl max-h-[85vh] overflow-hidden flex flex-col text-white xl:px-8 font-darker-grotesque ${themeClasses.modal}`}
      >
        <DialogHeader className="shrink-0">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-[32px] text-white font-normal">
              {stableTool?.name || "Run Tool"}
            </DialogTitle>
            {stableTool && renderStatusChip()}
          </div>
          <DialogDescription className="text-lg text-white font-normal">
            {stableTool
              ? (isInitialized &&
                  (mcpToolsCollection[stableTool.name] as MCPToolFromClient)
                    ?.description) ||
