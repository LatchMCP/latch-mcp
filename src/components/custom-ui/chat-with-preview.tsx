...
export default function ChatWithPreview({
  ...
  onSendMessage,
  onStop,
  previewUrl = "https://vercel-mcp-handler-latchmcp.vercel.app/mcp",
  ...
  const [steps, setSteps] = useState<DeployStep[]>([
    ...
    {
      key: "create_vercel_link",
      label: "Create Vercel import link",
      status: "pending",
    },
  ...
  const requiredEnvKeys = useMemo(() => {
    // Always require LATCHMCP essentials
    const mustHave = ["LATCHMCP_API_KEY", "LATCHMCP_API_URL"];
    try {
      ...
      for (const filePath of candidates) {
        ...
        for (const k of extractEnvKeysFromContent(content))
          discoveredKeys.add(k);
      }

      const deduped = Array.from(discoveredKeys);
      // Merge discovered keys with required LATCHMCP keys (LATCHMCP first for UX)
  ...
  function generateClientApiKey(prefix: string = "latchmcp"): string {
  ...
  function deriveApiUrl(): string {
    // Always use current website origin for LATCHMCP_API_URL
  ...
  async function handleDeploy() {
    ...
    for (const key of requiredEnvKeys) {
      if (key === "LATCHMCP_API_KEY") preparedEnv[key] = generateClientApiKey();
      else if (key === "LATCHMCP_API_URL") preparedEnv[key] = deriveApiUrl();
      else preparedEnv[key] = "";
    }

    // If user is authenticated, create a real API key via our API
    try {
      if (session?.user?.id && requiredEnvKeys.includes("LATCHMCP_API_KEY")) {
        ...
        if (
          ...
          preparedEnv.LATCHMCP_API_KEY = resp.apiKey as string;
        }
      }
    } catch (_) {
    ...
    setEnvCopied(
      Object.fromEntries(requiredEnvKeys.map((k) => [k, false])) as Record<
        string,
        boolean
      >
    );
    ...
    setSteps([
      ...
      {
        key: "redirect",
        label: "Open Vercel (after copying envs)",
        status: "pending",
      },
    ]);
    ...
    try {
      ...
      const res = await fetch("/api/deploy", {
        ...
        body: JSON.stringify({
          codebase,
          repoName: "latchmcp-app",
          isPrivate: true,
          // Always include LATCHMCP essentials in the env list sent to the deploy system
          env: Array.from(
            new Set([
              ...requiredEnvKeys,
              "LATCHMCP_API_KEY",
      ...
      <Dialog open={isDeployOpen} onOpenChange={setIsDeployOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              We’ll create a GitHub repository, commit your code, and send you
              to Vercel.
            </DialogDescription>
          </DialogHeader>
