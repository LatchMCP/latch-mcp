...
export class CDPSigningStrategy implements PaymentSigningStrategy {
    ...
    private async signWithCDPWallet(
        ...
        try {
            ...
            //const cdpAccount: string = await getCDPSolanaAccount(accountId, network);  // TODO change
            const cdpAccount = await getCDPAccount(accountId, network);  // TODO change
            console.log("[CDP Strategy] CDP Account", cdpAccount, network)
            ...
            // SOLANA_COMMENT
            //const keyPair: KeyPairSigner = await createKeyPairSignerFromBytes(bs58.decode(cdpAccount))

            //const signedPayment = await createPaymentHeaderSolana(keyPair, x402Version, paymentRequirement)
