/**
 * Database Actions for latchmcp.app
 * 
...
import { createCDPAccount, createCDPSolanaAccount } from '@/lib/gateway/3rd-parties/cdp';
    ...
    autoCreateCDPWalletForUser: (userId: string, userInfo: {
        ...
        try {
            ...
            const accountName = userInfo.displayName
                ? `latchmcp-${userInfo.displayName.toLowerCase().replace(/[^a-z0-9]/g, '-').slice(0, 10)}-${timestamp}`
                : `latchmcp-${userId.slice(0, 8)}-${timestamp}`;
