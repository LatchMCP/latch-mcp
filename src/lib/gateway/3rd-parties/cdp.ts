/**
 * Coinbase Developer Platform (CDP) Integration for latchmcp.app
 * 
...
import { SupportedSolanaNetworks } from "@/types/x402";
 ...
 */
export async function createCDPSolanaAccount(options: CreateCDPWalletOptions = {}): Promise<CDPWalletResult> {
    console.log("[CDP] Starting createCDPSolanaAccount with options:", options);

    const cdp = getCDPClient();
    console.log("[CDP] Got CDP client");

    const accountName = options.accountName || `latchmcp-account-${randomUUID()}`;
    ...
    try {
        ...
        console.log("[CDP] Main account solana created successfully");
...
export async function getCDPAccount(accountName: string, network: CDPNetwork = "base-sepolia") {
    ...
    try {
        console.log(`Getting CDP account ${accountName} for network ${network} ${SupportedSolanaNetworks}`);
        return cdp.evm.getOrCreateAccount({
            name: accountName
        })
    } catch (error) {
...
export async function getCDPSolanaAccount(accountName: string, network: CDPNetwork) {
    const cdp = getCDPClient();
    ...
    try {
        ...
        if (!account) {
            throw new Error(`Not found wallet ${accountName}`)
        }

        return cdp.solana.exportAccount({
...
export const CDP = {
    ...
    createAccountSolana: createCDPSolanaAccount,
