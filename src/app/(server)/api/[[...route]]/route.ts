/**
 * Main API handler for latchmcp.app
 * 
 * This module handles general API endpoints for the latchmcp.app platform.
...
app.post('/users/:userId/wallets/cdp', authMiddleware, async (c) => {
    try {
        ...
        const cdpOptions: CreateCDPWalletOptions = {
            accountName: data.accountName || `latchmcp-${user.id}-${Date.now()}`,
