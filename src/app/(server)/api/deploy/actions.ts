...
export async function deployWithGitHub(input: DeployRequestInput, userId: string, userLogin?: string): Promise<DeployResult> {
  // Ensure LATCHMCP keys are always included in env list passed to Vercel
  const ensuredEnv = Array.from(new Set<string>([...(input?.env || []), 'LATCHMCP_API_KEY', 'LATCHMCP_API_URL']));
  ...
  if (safeInput?.repositoryUrl) {
    ...
    params.set('project-name', safeInput.projectName || 'latchmcp-app');
  ...
  const desiredName = safeInput.repoName || 'latchmcp-app';
  ...
  const created = await createRepository(token, {
    ...
    description: 'Created by latchmcp Build',
  });
  ...
  for (const filePath of filePaths) {
    ...
    await putFileViaContentsApi(token, owner, repo, filePath, entry.content, defaultBranch, 'Initial import from latchmcp Build');
  }
